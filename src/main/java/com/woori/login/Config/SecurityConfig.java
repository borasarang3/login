package com.woori.login.Config;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests((auth -> {
            auth.requestMatchers("/").permitAll(); //각 매핑별 권한을 부여(메인페이지는 모든 사용자가 접근 가능)
            auth.requestMatchers("/result").authenticated(); //result는 로그인한 사용자만 권한부여

        }));

        //로그인 폼 정보
        //1. loginPage("매핑명") : 로그인페이지로 이동할 매핑명
        ...
        http.formLogin(login -> login
                ...
                .permitAll()
        ); //로그인폼은 기본폼 사용, 성공시 result맵핑처리, 로그인은 모든이용자가 접근가능
